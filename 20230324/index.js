...
// 비동기 함수
// 매초마다 동작하는 함수를 만들어보자
// 정한 시간마다 동작하는 함수 매개변수로 전달한 시간마다 동작한다.
// setInterval 제거 방법
let setTime = setInterval(() => {
    let date2 = new Date();
    // 값이 들어온것 객체가 아니고 1초마다 실행
...
function times(time){
    ...
    // 받아온 시간 중 하루 단위를 나눠서 나머지를 아래로
    time %= (24 * 60 * 60 * 1000);
    let hour = Math.floor(time / (60 * 60 * 1000));
    // 위에서 받은 시간 중 시간 단위를 나눠서 나머지를 아래로
    time %= (60 * 60 * 1000);
    let min = Math.floor(time / (60 * 1000));
    // 위에서 받은 시간 중 분 단위를 나눠서 나머지를 아래로
    time %= (60 * 1000);
    ...
    // console.log(sec);
    if(time < 1000){
        // setInterval 제거 함수 : 매개변수로 미리 설정해둔 함수의 변수를 적용한다.
